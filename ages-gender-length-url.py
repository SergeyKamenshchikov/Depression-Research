...
file_path=absolute_path+'/txt/links-unique.txt';#create absolute path
...
for i in range(start,mes):
  ...
  if int(age_content)>10 and (gen!='undefined') and int(length)>100:#filters of age,gender,length of message
    ...
    file_path=absolute_path+'/txt/age-gender-length-url.txt';file=open(file_path,'a',encoding='utf-8');
